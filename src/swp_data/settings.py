...
# Repo-root default config file. Overridable via SWP_CONFIG_FILE.
_DEFAULT_CONFIG_FILE = Path(__file__).resolve().parents[2] / "config.yaml"
...
def load_settings(config_file: Path | None = None) -> Settings:
    """Build Settings from config.yaml (if present) then environment overrides.

    Precedence: dataclass defaults < config.yaml < environment variables.
    A missing config file is fine; the dataclass defaults stand in.
    """
    values: dict = {}
    valid = {f.name for f in fields(Settings)}

    path = config_file or Path(os.environ.get("SWP_CONFIG_FILE", _DEFAULT_CONFIG_FILE))
    if path.exists():
