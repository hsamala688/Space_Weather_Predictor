"""PyTorch Dataset for Falisha's GL23x45 windowed dTEC dataset.
...
class FalishaDTECDataset(Dataset):
    ...
    def __init__(self, root: str | Path = "data/falisha_windows_gl23x45",
...
def make_falisha_dataloader(root: str | Path = "data/falisha_windows_gl23x45",
                            split: Split = "train",
                            batch_size: int = 16,
    ...
    dataset = FalishaDTECDataset(root=root, split=split)
    if shuffle is None:
        shuffle = split == "train"
...
if __name__ == "__main__":
    ds = FalishaDTECDataset(split="train")
    sample = ds[0]
    print(f"n={len(ds)}")
    print({key: tuple(value.shape) for key, value in sample.items() if key != "timestamp"})
    print(f"timestamp={sample['timestamp'].item()}")
